...
//		-------------------------------------------------------------------------------------------------
		...
		//price
		strPrice = request.getParameter("price").trim();
...
//		---------------------------------------
		...
		//price
		if (intPrice < 0) {
			errorMsg.put("price", "價錢不能是負數");
		}
		
		// location
		if (location.equals("") || location.trim().length() == 0) {
			errorMsg.put("location", "地點不能空白");
		}
...
//		---------------------------------------------------
		...
		Activity dis = new Activity(null, title, content, postTime, author, coverImage, coverImageName, deadline, null, intPrice, location);
		Integer activityId = ds.saveActivity(dis);
//		---------------------------------------------------

		
		// 5. 轉到頁面
		String newpage = "activity?d=" + activityId;
